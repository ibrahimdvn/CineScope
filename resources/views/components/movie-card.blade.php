@php
    ...
    $isTv     = $isModel
        ...
        : (isset($movie['media_type']) && $movie['media_type'] === 'tv');
    $routeUrl = $isTv ? route('tv.show', $tmdbId) : route('movies.show', $tmdbId);
    ...
    $poster   = $isModel ? $movie->poster_path : ($movie['poster_path'] ?? null);
    $vote     = $isModel ? $movie->vote_average : ($movie['vote_average'] ?? 0);
@endphp
<a href="{{ $routeUrl }}" class="movie-card">
    ...
    <div class="movie-info">
        ...
        <div class="movie-meta">
            <span>{{ $date ? substr($date, 0, 4) : 'N/A' }}</span>
